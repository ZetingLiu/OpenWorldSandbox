#!/bin/bash
# MiniMax-M3 Agent 解题评测：协议对齐 glm-5.1 / deepseek-v4-pro（7 任务 × 3 runs = 21 runs），
# 结束后自动 fold 进 outputs/runs/baseline/leaderboard.json。
# 密钥全部来自 .env 的 OWB_MINIMAX_M3_*，不在脚本中出现。
...
# Load .env first, then override the OpenAI-compatible config with MiniMax's.
if [[ -f .env ]]; then
...
if [[ -z "$OPENAI_BASE_URL" || -z "$OPENAI_API_KEY" ]]; then
    echo "error: OWB_MINIMAX_M3_BASE_URL / OWB_MINIMAX_M3_API_KEY not set in .env" >&2
    exit 1
fi
...
echo "Model:   $MODEL"
echo "Runs:    $RUNS per task (protocol aligned with glm-5.1 / deepseek-v4-pro: 7 tasks × 3 runs)"
echo "Output:  $OUTDIR"
echo ""
...
# ── 2. Run each task RUNS times (7 tasks, same set as glm-5.1 / deepseek) ──
TASKS=(
...
"$PY" scripts/update_leaderboard.py \
    ...
    --report "$REPORT" \
    --tasks_dir data/tasks \
    --note "协议对齐 glm-5.1 / deepseek-v4-pro：7 任务 × 3 runs"
